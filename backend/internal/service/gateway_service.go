	...
	if len(routingAccountIDs) == 0 && sessionHash != "" && stickyAccountID > 0 && !isExcluded(stickyAccountID) {
		...
		if accountID > 0 && !isExcluded(accountID) {
			...
			if ok {
				...
				// accountByID is built from the already group-filtered candidate list.
				// Cached snapshot accounts may not always carry AccountGroups, so
				// repeating isAccountInGroup here can incorrectly miss a valid sticky hit.
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	// 处理可切换账号的错误（状态码级 + 账号级 4xx：订阅过期/配额/计费）
	if resp.StatusCode >= 400 {
		respBody, _ := io.ReadAll(io.LimitReader(resp.Body, 2<<20))
		_ = resp.Body.Close()
		resp.Body = io.NopCloser(bytes.NewReader(respBody))
		...
		if s.shouldFailoverUpstreamError(resp.StatusCode) || isUpstreamAccountStateError(resp.StatusCode, upstreamMsg, respBody) {
			...
			if reason := accountStateFailoverReason(resp.StatusCode, upstreamMsg, respBody); reason != "" {
...
func (s *GatewayService) handleBedrockUpstreamErrors(
	...
	// non-retryable failover (status-coded + account-level 4xx)
	if resp.StatusCode >= 400 {
		respBody, _ := io.ReadAll(io.LimitReader(resp.Body, 2<<20))
		_ = resp.Body.Close()
		resp.Body = io.NopCloser(bytes.NewReader(respBody))
