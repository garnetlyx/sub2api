...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_ReturnsFailoverCandidateAfterInitialReadFailure(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	case serverErr := <-serverErrCh:
		...
		require.True(t, IsOpenAIWSIngressAccountFailoverCandidate(serverErr))
		require.Equal(t, 2, dialer.DialCount(), "same-account retry should be exhausted before handler-level account failover")
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket 结束超时")
	}
}
