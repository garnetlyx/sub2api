...
const (
	...
	// CodexClientRestrictionReasonMatchedCodexHeader 表示请求命中 Codex Responses 协议头。
	CodexClientRestrictionReasonMatchedCodexHeader = "official_client_codex_header_matched"
...
func (d *OpenAICodexClientRestrictionDetector) Detect(c *gin.Context, account *Account) CodexClientRestrictionDetectionResult {
	...
	if c != nil && c.Request != nil && openai.HasCodexOfficialClientRequestHeader(c.Request.Header) {
		return CodexClientRestrictionDetectionResult{
			Enabled: true,
			Matched: true,
			Reason:  CodexClientRestrictionReasonMatchedCodexHeader,
		}
	}
