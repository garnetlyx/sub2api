...
type RateLimitConfig struct {
	...
	// AccountStateCooldownMinutes is the temporary-unschedulable cooldown for
	// account-level upstream errors (lapsed subscription, quota exhausted,
	// billing problems reported as 400/402). The account stays schedulable in
	// status (never permanently disabled) so it self-recovers once its state
	// is fixed upstream (renewal, top-up).
	AccountStateCooldownMinutes int `mapstructure:"account_state_cooldown_minutes"`
}
...
func setDefaults() {
	...
	viper.SetDefault("rate_limit.account_state_cooldown_minutes", 30)
