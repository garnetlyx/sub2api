...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		reqLog.Debug("openai.account_schedule_decision",
			...
			zap.Any("provider_counts", scheduleDecision.ProviderCounts),
			zap.Int("top_k", scheduleDecision.TopK),
			zap.Int64("latency_ms", scheduleDecision.LatencyMs),
			zap.Float64("load_skew", scheduleDecision.LoadSkew),
			zap.String("selected_provider", scheduleDecision.SelectedProvider),
		)
		...
		reqLog.Debug("openai.account_selected", zap.Int64("account_id", account.ID), zap.String("account_name", account.Name), zap.String("provider", account.ProviderIdentity()))
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				if switched {
					...
					reqLog.Warn("openai.compatibility_failover_retrying",
	...
	for {
		...
		if err != nil {
			...
			if len(fs.FailedAccountIDs) == 0 {
				...
				if err != nil {
					if compatibilityFallbackSourceErr != nil {
						h.handleAnthropicFailoverExhausted(c, compatibilityFallbackSourceErr, streamStarted)
						return
					}
		...
		reqLog.Debug("openai_messages.account_selected", zap.Int64("account_id", account.ID), zap.String("account_name", account.Name), zap.String("provider", account.ProviderIdentity()))
		setOpsSelectedAccount(c, account.ID, account.Platform)
		...
		defaultMappedModel := resolveOpenAIForwardDefaultMappedModel(currentAPIKey, c.GetString("openai_messages_fallback_model"))
		// 应用渠道模型映射到请求体
		forwardBody := body
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				if switched {
					currentAPIKey = fallbackAPIKey
					currentSubscription = nil
					currentChannelMappingMsg = fallbackChannelMapping
					compatibilityFallbackUsed = true
					compatibilityFallbackSourceErr = failoverErr
					fs = NewFailoverState(h.maxAccountSwitches, false)
					loadCompatibilityExcludedPlatforms(c.Request.Context(), reqLog, h.gatewayService, currentAPIKey.GroupID, compatibilityScopeKey, fs, "openai_messages.compatibility_exclusion_cache_hit")
					...
					fs.LastFailoverErr = failoverErr
					h.gatewayService.RecordOpenAIAccountSwitch()
					reqLog.Warn("openai_messages.compatibility_failover_retrying",
						zap.String("requested_model", routingModel),
						zap.String("canonical_model", service.CanonicalizePublicModel(routingModel)),
						zap.String("compatibility_category", failoverErr.CompatibilityCategory),
						zap.Any("fallback_group_id", currentAPIKey.GroupID),
					)
					continue
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	ensureInitialUserSlot := func() bool {
		if currentUserRelease != nil {
			return true
		}
	...
	canSwitchAccount := func(account *service.Account, reason string, err error) bool {
		if account == nil || previousResponseID != "" {
			return false
		}
	...
	for {
		...
		if err != nil {
			...
			releaseTurnSlots()
			if canSwitchAccount(account, "access_token", err) {
				continue
			}
		...
		reqLog.Debug("openai.websocket_account_selected",
			zap.Int64("account_id", account.ID),
			zap.String("account_name", account.Name),
			zap.String("provider", account.ProviderIdentity()),
		...
		if err := h.gatewayService.ProxyResponsesWebSocketFromClient(ctx, c, wsConn, account, token, wsFirstMessage, hooks); err != nil {
			...
			releaseTurnSlots()
			if service.IsOpenAIWSIngressAccountFailoverCandidate(err) &&
				canSwitchAccount(account, service.OpenAIWSIngressAccountFailoverReason(err), err) {
				continue
			}
