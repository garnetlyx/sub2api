...
func (h *KiroOAuthHandler) StartDeviceFlow(c *gin.Context) {
	...
	result, err := h.kiroOAuthService.StartDeviceFlow(c.Request.Context(), req.Region, req.ProxyID)
	if err != nil {
...
func (h *KiroOAuthHandler) PollDeviceFlow(c *gin.Context) {
	...
	result, err := h.kiroOAuthService.PollDeviceFlow(c.Request.Context(), req.SessionID, req.ProxyID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *KiroOAuthHandler) CreateFromDeviceFlow(c *gin.Context) {
	var req struct {
		SessionID   string  `json:"session_id" binding:"required"`
		ProxyID     *int64  `json:"proxy_id"`
		Name        string  `json:"name"`
	...
	existing, err := h.kiroOAuthService.FindBySubject(c.Request.Context(), result.Subject)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
func (h *KiroOAuthHandler) CreateFromImport(c *gin.Context) {
	var req struct {
		RefreshToken string  `json:"refresh_token" binding:"required"`
		Region       string  `json:"region"`
		ProxyID      *int64  `json:"proxy_id"`
